...
export const Debug = (function () {
    return class Debug {
        ...
        static async test (game) {
            if (Levels.AVAILABLE_LEVELS.every((level, index) => index === 0 ? true : level === false)) {
                for (let i = 0; i < Levels.MAX_LEVEL; i++) {
                    Levels.setStageAvailable(i + 1);
                }
            } else {
                Levels.resetAvailableLevels();
            }

            game.levelSelectScreen.refreshStageStates();
