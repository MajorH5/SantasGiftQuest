...
import { GiftGun } from "../weapons/giftGun.js";
    ...
    return class Player extends Entity {
        ...
        damage (amount, source) {
            ...
            if (wasSuccessfulDamage) {
                this.lastDamaged = this.getElapsedTime();
            ...
            document.addEventListener('keydown', (event) => {
                ...
                switch (key) {
                    ...
                    case 'w':
                        ...
                        this.keys[key] = this.getElapsedTime();
                        if (!this.motionClimb()) {
                            // failed to climb, try to jump

                            if (!this.motionJump() && this.isSpawned && this.isAlive && this.bufferJumps < 1) {
                                // failed to jump, maybe buffer jump?
                                const jumpAttemptTime = this.getElapsedTime();
                                ...
                                this.onLand.listenOnce(() => {
                                    ...
                                    const landTime = this.getElapsedTime();
                                    const elapsedSeconds = (landTime - jumpAttemptTime) / 1000;

                                    // non-negative check incase world reset
                                    if (elapsedSeconds > 0 && elapsedSeconds < this.bufferTime) {
                                        this.motionJump();
                                    }
                                });
                            }
                    ...
                    case 's':
                        ...
                        const didDisable = this.disableSemiSolids();

                        const disableAttemptTime = this.getElapsedTime();

                        if (!didDisable && this.isSpawned && this.isAlive && this.bufferDrops < 1) {
                            this.bufferDrops++;
                            ...
                            this.onLand.listenOnce(() => {
                                ...
                                const landTime = this.getElapsedTime();
                                const elapsedSeconds = (landTime - disableAttemptTime) / 1000;

                                if (elapsedSeconds < this.bufferTime) {
        ...
        canCoyote () {
            ...
            if (!this.onPlatform && !this.isClimbing && !this.isJumping) {
                const elapsedSinceLeftFloor = (this.getElapsedTime() - this.timeLeftPlatform) / 1000;
                const elapsedSinceDisableSemiSolid = (this.getElapsedTime() - this.timeDisableSemiSolid) / 1000;
        ...
        onHostileEntityCollision (entity) {
            ...
            if (entity.squashable && cameDown && landedOnTop) {
                ...
                let elapsed = this.getElapsedTime() - (this.keyHeld('arrowup') || this.keyHeld('w') || -Infinity);
        ...
        handleEntityCollision (entity) {
            if (entity.hostile && entity.sprite.visible) {
        ...
        handleTileCollision (tile) {
            ...
            if (tile.spriteIndex === Tile.SPIKE && tile.sprite.visible && !this.invulnerable) {
        ...
        update (deltaTime) {
            ...
            if (this.health < this.maxHealth && this.health > 0) {
                const elapsedSinceLastRegen = this.getElapsedTime() - this.lastRegen;
    
                if (elapsedSinceLastRegen > this.regenRate) {
                    this.lastRegen = this.getElapsedTime();
            ...
            if (this.onPlatform && !this.body.floored && !this.isClimbing && !this.isJumping) {
                ...
                this.timeLeftPlatform = this.getElapsedTime();
        ...
        render (context, offset, scale) {
            const elapsed = (this.getElapsedTime() - this.lastDamaged);
