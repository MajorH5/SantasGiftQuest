import { Projectile } from "../../../world/gameobjects/projectiles/projectile.js";
...
export const Entity = (function () {
    return class Entity extends GameObject {
        ...
        constructor (spriteTexture, spriteSize, bodyProperties, maxHealth) {
            ...
            this.climbInterval = null; // the interval for climbing
        ...
        damage (amount, source = null) {
            ...
            if (this.health === 0) {
                ...
                this.isAlive = false;
                this.deathTime = this.getElapsedTime();
        ...
        motionWalk (direction) {
            ...
            let timeSave = this.getElapsedTime();
            let movingUpdate = () => {
                ...
                let elapsed = this.getElapsedTime() - timeSave;
        ...
        motionSlow () {
            ...
            // start slowing down
            let slowStart = this.getElapsedTime();
            let slowInterval = () => {
                if (this.walkingDirection !== 0 || Math.abs(this.body.velocity.x) === 0 || this.isDead()) {
                    // stop slowing down if the direction has changed
                    this.onUpdate.unlisten(slowInterval);
                    return;
                }

                let elapsedScale = (this.getElapsedTime() - slowStart) / 1000 * acceleration * 2;
            ...
            this.onUpdate.listen(slowInterval);
            slowInterval();
        }
        ...
        canClimb () {
            return this.isSpawned && this.climbInterval === null && !this.isClimbing && this.touchingClimbable() !== null;
        }
        ...
        motionClimb () {
            ...
            let climbInterval = () => {
                if (this.touchingClimbable() === null || !this.isClimbing || this.isDead()) {
                    ...
                    this.onUpdate.unlisten(climbInterval);
            ...
            this.onUpdate.listen(climbInterval);
            climbInterval();

            this.climbInterval = climbInterval;
        ...
        disableSemiSolids () {
            ...
            this.timeDisableSemiSolid = this.getElapsedTime();
        ...
        // is to despawning
        getDespawnTimer () {
            let despawnPercent = (this.getElapsedTime() - this.deathTime) / 1000 / this.despawnDelay;
        ...
        render (context, offset, scale) {
            ...
            if (this.behavior !== null) {
                this.behavior.render(context, offset, scale);
            }
        }
    }
})();