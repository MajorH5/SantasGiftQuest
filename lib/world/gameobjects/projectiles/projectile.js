...
export const Projectile = (function () {
    ...
    return class Projectile extends GameObject {
        ...
        constructor (spawnPosition, velocity, spriteIndex, damage, speed, fromPlayer, source) {
            ...
            this.renderPriority = Constants.PROJECTILE_RENDER_PRIORITY;
            this.damage = damage;
        ...
        onCollision (other) {
            ...
            if (!entity || entity.isDead() || !entity.sprite.visible || entity === this.soruce){
                // no entity, or its dead, or its invisible, or its the source
                return false;
            }
            
            if (this.fromPlayer && !entity.hostile) {
                // don't let player hit non-hostile entities
                return false;
            }
