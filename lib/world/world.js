import { UIFlamethrowerCutsceneScreen } from '../ui/screens/uiFlamethrowerCutsceneScreen.js';
import { SnowmanBoss } from './gameobjects/entities/enemies/snomwanBoss.js';
import { UIGiftIndicators } from '../ui/features/uiGiftIndicators.js';
import { GameObjectManager } from './gameobjects/gameObjectManager.js';
import { ParticlesManager } from '../particles/particlesManager.js';
import { Physics } from '../PhysicsJS2D/src/PhysicsJS2D.js';
...
export const World = (function () {
    return class World {
        ...
        static PARALLAX_BACKGROUND_SCALE = 3;
        ...
        constructor (canvas, worldSize = new Vector2(0, 0), scale = Camera.DEFAULT_SCALE) {
            ...
            this.parallaxManager = new ParallaxManager(World.PARALLAX_BACKGROUND_SCALE);
        ...
        async loadStage (stage, isReload = false) {
            ...
            this.onAfterStageLoad(isReload);
        }
        ...
        async handlePlayerCollision (other) {
            ...
            switch (tile.spriteIndex) {
                case Tile.SLEIGH:
                    if (this.tileMap.getGiftCount() > 0 || this.player.isDead() || this.stageClearing) {
                        return;
                    }
        ...
        async clearStage (nextStage) {
            ...
            if (this.player.isDead()) {
                console.warn('Player is dead, rejecting clear request!');
                return
            }

            if (this.cutscene !== null) {
                console.warn('Cutscene is playing, rejecting clear request!');
                return;
            }
            ...
            if (!this.isDemoWorld) {
                Levels.setStageAvailable(this.stage);
                Levels.setStageAvailable(this.stage + 1);
            }
        ...
        resize (size) {
            ...
            for (let i = 0; i < backgroundLayers.length; i++) {
                ...
                // (canvasSize / scaleOfBGImages) - BGImageStandardSize

                layer.position.y = (canvasSize.y / parallaxImageScale) - 300 + this.worldSize.y * layer.parallaxScale.y
            }
        }
        ...
        // temporarily pauses the world execution
        pause () {
        ...
        // resumes any logic that was haulted from pause
        resume () {
            for (let i = 0; i < this.tweens.length; i++) {
                const tween = this.tweens[i];
        ...
        update (deltaTime) {
            ...
            this.parallaxManager.update(deltaTime);
        ...
        // creates a tween object and caches it
        // inside this world instance
        createTween (duration, tweenData, easing) {
            const tween = new Tween(duration, tweenData, easing);

            tween.started.listen(() => {
                this.tweens.push(tween);
            });

            tween.stopped.listen(() => {
        ...
        tweenScale (scale, durationSeconds = 1) {
            ...
            const tween = this.createTween(1000 * durationSeconds, [[this.scale, scale]])

            this.scaleTween = tween;
            
            return tween.begin((value) => {
                this.setScale(value);
            }).then((wasCompleted) => {
                if (wasCompleted) {
                    this.scaleTween = null;
