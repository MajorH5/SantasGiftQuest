...
export const ActionChase = (function () {
    ...
    return class ActionChase extends Behavior {
        ...
        climbingChase (targetPosition, targetBody, hostPosition, ladderBody) {
            ...
            const upRayCast = this.raycast(hostPosition, new Vector2(0, -1), Constants.TILE_SIZE, (body) => body.getTag('climbable'));
        ...
        verticalChase (targetPosition, hostPosition, horizontalDistance, verticalDistance, target) {
            ...
            if (verticalDistance > Constants.TILE_SIZE * 4 && target.body.velocity.y === 0) {
                ...
                const filter = (body) => body.getTag('climbable');

                let leftClimbable = this.raycast(hostPosition, new Vector2(-1, 0), Constants.TILE_SIZE * 5, filter);
                let rightClimbable = this.raycast(hostPosition, new Vector2(1, 0), Constants.TILE_SIZE * 5, filter);

                const climbable = leftClimbable || rightClimbable;

                if (climbable) {
                    this.searching = true;
                    this.host.motionWalk(climbable.position.x < hostPosition.x ? -1 : 1);
                } else {
                    // no climbable tiles found, lets see if we can gain height
                    const spaceAbovePosition = (body) => {
                        const spaceAbove = this.raycast(body.getCenter(), new Vector2(0, -1), Constants.TILE_SIZE * 3, (other) =>{
                            return other !== body && (other.solid || other.semiSolid);
                        });
                    ...
                    const solid = solidTileLeft || solidTileRight;
        ...
        perform (target){
            ...
            if (targetLower && hostLower) {
                if (targetLower.position.y === hostLower.position.y) {
