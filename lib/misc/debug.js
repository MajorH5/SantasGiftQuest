import { Reindeer } from '/lib/world/gameobjects/entities/friendlies/reindeer.js';
import { Elf } from '/lib/world/gameobjects/entities/friendlies/elf.js';
import { Entity } from '/lib/world/gameobjects/entities/entity.js';
...
export const Debug = (function () {
    return class Debug {
        ...
        render () {
            ...
            if (player !== null) {
                ...
                if (!(player.screenMousePosition.equals(new Vector2(-1, -1)))) {
                    const ray = mouseDirection.scale(Constants.TILE_SIZE * cameraScale).add(playerCenterScreen);

                    this.context.strokeStyle = 'red';
                    this.context.beginPath();
                    this.context.moveTo(playerCenterScreen.x, playerCenterScreen.y);
                    this.context.lineTo(ray.x, ray.y)
                    this.context.stroke();

                    const hitTile = tileMap.rayCast(player.body.getCenter(), mouseDirection, 12 * 5, () => true);

                    if (hitTile !== null) {
                        const tileBody = hitTile.body;

                        this.context.fillStyle = 'orange';
                        this.context.globalAlpha = 0.5;
                        this.context.fillRect(
                            tileBody.position.x * cameraScale + cameraOffset.x * cameraScale,
                            tileBody.position.y * cameraScale + cameraOffset.y * cameraScale,
                            tileBody.size.x * cameraScale,
                            tileBody.size.y * cameraScale
                        );
                    }
        ...
        static async test (game) {
            ...
            deer.setPosition(player.body.getCenter().subtract(new Vector2(0, 12 * 5)));
            world.spawn(deer);

            const elf = new Elf();
            elf.setPosition(player.body.getCenter().subtract(new Vector2(0, 12 * 5)));
            world.spawn(elf);

            // world.gameObjectManager.getGameObjects().forEach(element => {
            //     if (element instanceof player.constructor) {
            //         return;
            //     }

            //     element.damage(Infinity)
            // });

            // world.setScale(world.scale + 0.1);
        }
    }
})();