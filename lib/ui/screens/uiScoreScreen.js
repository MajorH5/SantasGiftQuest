...
import { UISleigh } from '../features/uiSleigh.js';
...
export const UIScoreScreen = (function () {
    return class UIScoreScreen extends UIBase {
        ...
        constructor () {
            ...
            this.exitButton.mouseUp.listen(() => {
                this.dropAndRotateFrames();
                this.onMenu.trigger()
            });
        ...
        //
        dropAndRotateFrames () {
            for (let i = 0; i < this.scoreFrames.length; i++) {
                const frame = this.scoreFrames[i];

                let rotationalVelocity =( 0.04 + Math.random() * 0.07) * (Math.random() < 0.5 ? -1 : 1);
                let fallVelocity = -(1 + Math.random() * 2);

                const onUpdate = () =>{
                    fallVelocity += 2;
                    rotationalVelocity *= 0.98;

                    frame.rotation += rotationalVelocity;
                    frame.positionAbsolute.y += fallVelocity;

                    if (frame.positionAbsolute.y > 2000) {
